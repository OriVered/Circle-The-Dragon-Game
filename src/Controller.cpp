...
{
	m_infoUndo = make_unique <InfoButton>("Undo", infoUndo);
	m_infoReset = make_unique <InfoButton>("Reset", infoReset);
	m_gameSteps = make_unique <Info>("Steps done: ", infoSteps, noSteps);
	m_level = make_unique <Info>("Level: ", infoLevel, firstLevel);
}
...
{
	if (m_infoReset->getGlobalBounds().contains(location))
		reset();
	else if (m_infoUndo->getGlobalBounds().contains(location))
		undo();
...
void Controller::render()
{
	...
	m_infoUndo->draw(m_window);
	m_infoReset->draw(m_window);
